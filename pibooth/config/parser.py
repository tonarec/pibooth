...
DEFAULT = odict((
    ...
    ("PRINTER",
        odict((
            ...
            ("default_copies",
                (1,
                 "Default number of copies sent to the printer",
                 'Default printed copies', [str(i) for i in range(1, 11)])),
            ("max_copies", # TODO: Use max_duplicates instead, but change the logic (no display on wait screen)
                (3,
                 "Maximum number of copies sent to the printer",
                 'Maximum of printed copies', [str(i) for i in range(1, 11)])),
            ("pictures_per_page",
                (1,
