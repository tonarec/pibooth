...
@hookspec
def state_copies_enter(cfg, app, win):
    """Actions performed when application enter in Copies state.
...
@hookspec
def state_copies_do(cfg, app, win, events):
    """Actions performed when application is in Copies state.
    This hook is called in a loop until the application can switch
    to the next state.
...
@hookspec(firstresult=True)
def state_copies_validate(cfg, app, win, events):
    """Return the next state name if application can switch to it
    else return None.
...
@hookspec
def state_copies_exit(cfg, app, win):
    """Actions performed when application exit Copies state.
